...
public class Net implements NetIf {
    ...
    private static final String URL = "http://abcdf.vaiwan.com";
    ...
    @Override
    public void setClock(Map<String, Integer> params, NetListener listener) {//设置闹钟
        String finalUrl = buildParams("/setNewClock", params);
    ...
    @Override
    public void deleteClock(int[] clockid, NetListener listener) {//删除闹钟
        int count = clockid.length;
        String finalUrl = URL + "/deleteClock?clockid=";
        String[] finalUrls = new String[count];
        for (int i = 0; i < count; i++) {
            finalUrls[i] = finalUrl + clockid[i];
        }
        for (int j = 0; j < count; j++) {
            ...
            ThreadExecutor.runBG(new Runnable() {
                @Override
                public void run() {
                    doGet(finalUrls[finalJ], listener);
                }
            });
        }
    }
